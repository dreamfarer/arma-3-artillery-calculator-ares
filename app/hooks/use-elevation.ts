...
export function useElevation() {
  const { mapMetadata, activeMap } = useMapContext();

  return useCallback(
    async (point2d: Point2D): Promise<number> => {
      if (!mapMetadata || !activeMap) return 0;
      const meta = mapMetadata[activeMap];
      const isDev = process.env.NODE_ENV === 'development';
      const url = isDev ? meta.elevationDevUrl : meta.elevationProdUrl;
      ...
      const valueResp = await fetch(url, {
        headers: {
